...
VERSION=$(GITTAG:v%=%)
LINKER_FLAGS=-s -w -X 'github.com/jollytachome/terraform-provider-couchbase-capella/version.ProviderVersion=${VERSION}'
...
fmt:
	...
	find . -name "*.go" -exec goimports -w -local github.com/jollytachome/terraform-provider-couchbase-capella {} \;
